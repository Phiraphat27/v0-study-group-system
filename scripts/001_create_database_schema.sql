...
-- 3. Create Indexes (ใช้ IF NOT EXISTS เพื่อป้องกัน Error)
CREATE INDEX IF NOT EXISTS idx_profiles_student_id ON public.profiles(student_id);
CREATE INDEX IF NOT EXISTS idx_user_skills_user_id ON public.user_skills(user_id);
CREATE INDEX IF NOT EXISTS idx_user_skills_subject_id ON public.user_skills(subject_id);
...
CREATE INDEX IF NOT EXISTS idx_exam_schedules_user_id ON public.exam_schedules(user_id);

-- 4. Create Trigger Function (ใช้ OR REPLACE ทับของเดิมได้)
...
-- 5. Create Triggers (ลบของเดิมก่อนสร้างใหม่ เพื่อป้องกัน Error ซ้ำ)
